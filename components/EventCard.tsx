...
const EventCard: React.FC<EventCardProps> = ({ event, onClick }) => {
  ...
  // CSS variables for mobile color reveal
  const mobileColorVars = {
    '--ring-active-color': isFire ? 'rgba(255, 138, 0, 0.2)' : 'rgba(0, 198, 255, 0.2)',
    '--text-active-color': isFire ? '#FF8A00' : '#00C6FF'
  } as React.CSSProperties;

  return (
    <div
      onClick={() => onClick(event)}
      className={`mobile-color-reveal group relative overflow-hidden rounded-[1.5rem] md:rounded-[2.5rem] glass cursor-pointer transition-all duration-700 ease-out ${glowClass} hover:-translate-y-3 md:hover:-translate-y-4 hover:scale-[1.01] hover:shadow-2xl`}
      style={mobileColorVars}
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className={`absolute -top-10 -right-10 w-48 h-48 md:w-64 md:h-64 rounded-full blur-[80px] md:blur-[100px] opacity-10 transition-all duration-1000 glow-mobile group-hover:opacity-40 group-hover:scale-125 ${isFire ? 'bg-fire' : 'bg-ice'}`}></div>
      ...
      <div className="h-56 md:h-64 overflow-hidden grayscale grayscale-mobile group-hover:grayscale-0 transition-all duration-1000 relative">
        <img
          src={event.image}
          alt={event.title}
          className="w-full h-full object-cover transform scale-100 scale-mobile group-hover:scale-115 transition-transform duration-[4s] ease-out"
        />

        <div className={`absolute inset-0 transition-all duration-1000 overlay-mobile group-hover:opacity-40 opacity-0 ${isFire ? 'bg-gradient-to-tr from-fire-dark/80 via-transparent to-transparent' : 'bg-gradient-to-tr from-ice-dark/80 via-transparent to-transparent'}`}></div>
      ...
      <div className="p-6 md:p-8 relative z-10 bg-brand-alt/40 backdrop-blur-sm">
        ...
        <div className="flex items-center justify-between pt-4 md:pt-6 border-t border-white/5">
          ...
          <div className="flex space-x-1 md:space-x-1.5">
            <div className={`w-1 h-1 rounded-full transition-all duration-300 dot-mobile ${isFire ? 'bg-fire/20 group-hover:bg-fire' : 'bg-ice/20 group-hover:bg-ice'}`}></div>
            <div className={`w-1 h-1 rounded-full transition-all duration-500 dot-mobile ${isFire ? 'bg-fire/20 group-hover:bg-fire' : 'bg-ice/20 group-hover:bg-ice'}`}></div>
            <div className={`w-1 h-1 rounded-full transition-all duration-700 dot-mobile ${isFire ? 'bg-fire/20 group-hover:bg-fire' : 'bg-ice/20 group-hover:bg-ice'}`}></div>
          </div>
        </div>

        {/* Animated Bottom Sync Bar */}
        <div className="absolute bottom-0 left-0 w-full h-[1.5px] bg-white/5 overflow-hidden">
          <div className={`h-full w-0 bar-mobile transition-all duration-[1s] ease-in-out group-hover:w-full ${isFire ? 'bg-fire shadow-[0_0_20px_#FF8A00]' : 'bg-ice shadow-[0_0_20px_#00C6FF]'}`}></div>
        </div>
      </div>
    </div>
  );
